...
def test_category_empty(middle_price):
    assert middle_price == ''
    assert middle_price == 1
...
def test_invalid_product(product_invalid):
    prod_inv = product_invalid
    assert prod_inv == Product("Бракованный товар", "Неверное количество", 1000.0, 0)


def test_cat_empty(category_empty):
    cat_empty = category_empty
    assert cat_empty == Category("Пустая категория", "Категория без продуктов", [])
